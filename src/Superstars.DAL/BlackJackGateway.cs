...
namespace Superstars.DAL
{
    public class BlackJackGateway
    {
        ...
        public async Task<Result<int>> CreateJackPlayer(int userId, string[] cards = null)
        {
        ...
        public async Task<Result<int>> CreateJackAi(int userId, string[] cards = null)
        {
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                var p = new DynamicParameters();
        ...
        public async Task<BlackJackData> GetGameId(int playerId)
        {
            ...
            {
                return await con.QueryFirstOrDefaultAsync<BlackJackData>(
                    "select top 1 t.BlackJackGameId from sp.vBlackJackPlayer t where t.BlackJackPlayerId = @BJPlayerId order by BlackJackGameId desc",
                    new { BJPlayerId = playerId });
            }
        }
