  ...
  export default {
    ...
    async RedirectandDelete() {
      ...
      getDiceIaImage(value, index) {
        let image = this.indexRerollDicesIa.findIndex(x => x === index) !== -1 && this.iaRollDices ? `diceIaRoll.gif` :
          `diceia${value}.png`;
        return require(`../img/${image}`);
      },

      getDiceImage(value, index) {
        let image = this.selected.findIndex(x => x === index + 1) !== -1 && this.rollDices ? `dicePlayerRoll.gif` :
          `dice${value}.png`;
        return require(`../img/${image}`);
      }
    }
