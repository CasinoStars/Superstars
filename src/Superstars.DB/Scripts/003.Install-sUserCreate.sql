...
begin
    ...
    insert into sp.tUser(UserName, UserPassword, Email, PrivateKey, Country, IsAdmin) values(@UserName, @UserPassword, (case when @Email is null then '' else @Email end),@PrivateKey, @Country, 0);
	...
	insert into sp.tStats(GameTypeId, UserId, Wins, Losses) values(1, @UserId, 0, 0);
	end;
	commit;
	return 0;
end;