...
namespace CodeCake
{
    ...
    {
        private void StandardCreateZip(DirectoryPath releasesDir, IEnumerable<SolutionProject> projectsToPublish,
            SimpleRepositoryInfo gitInfo, string configuration)
        {
            ...
            foreach (var p in projectsToPublish)
            {
                if (p.Name == "Superstars.WebApp")
                {
                    ...
                    Cake.Zip(p.Path.GetDirectory() + "/bin/Debug/netcoreapp2.1/publish",
                        "WebApp." + gitInfo.SafeSemVersion + ".zip");
                }
